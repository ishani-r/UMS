               ...
               <div class="card-content">
                  <div class="card-body">
                     ...
                     <div class="alert alert-danger">
                        <a href="{{route('change_password')}}" class="close" data-dismiss="alert" aria-label="close">x</a>
                        {{ session('error') }}
                     </div>
                     @endif
                     @if (session('success'))
                     ...
                     @endforeach
                     @endif
                     <form method="post" action="{{ route('change_password') }}" id="changePassword">
                        @csrf
                        <div class="row">
                           <div class="col-md-6">
                              <fieldset class="form-group">
                                 <label for="basicInput">Corrent Password</label>
                                 <input type="password" class="form-control" name="current-password" value="{{ old('current-password') }}" placeholder="Enter Your Corrent Password">
                              </fieldset>
                              ...
                              <fieldset class="form-group">
                                 <button type="submit" class="btn gradient-pomegranate big-shadow">Change
                                    password</button>&nbsp;&nbsp;
                                 <a href="{{ route('college.main') }}" type="submit" class="btn gradient-mint shadow-z-4">Go Back!</a>
                              </fieldset>
                           </div>
                        </div>
                     </form>
